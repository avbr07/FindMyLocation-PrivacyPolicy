...
- **While you are typing, nothing is sent at all.** From version 1.20.1 the search box offers suggestions as you type — towns, cities and well-known places — and those come from lists that ship inside the App. The letters you type are matched on your device against the on-device place list (**GeoNames**) and the bundled list of attractions and landmarks (**OpenStreetMap**), and your own position is used, on the device only, to put nearer places first. **No keystroke, no search text and no coordinate leaves your phone for this**, it works with no connection, and it does not count towards the daily limit described below. Only when you then run a search does the next point apply.
