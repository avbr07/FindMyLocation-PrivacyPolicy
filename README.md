...
- If you tap the Map tab's **fuel (Petrol/Gas)**, **Food**, **Hospital** or **Attractions** buttons, your coordinates and chosen search radius are sent to the **Overpass API** (an OpenStreetMap community service) to find nearby fuel stations, restaurants, medical services and attractions.
    - **Changed in version 1.18.0 — famous places no longer involve the internet.** The well-known attractions layer (temples, monuments, museums and the like) now comes from a list of notable places shipped **inside the App** (**Wikidata**, CC0), so it is worked out entirely on your device. Your coordinates are **no longer sent to the Wikidata Query Service** for this. The App still asks the **Overpass API** for the smaller, more local places around you, and for their exact positions.
    - The only exception: if that shipped list cannot be read on your device — for example there is not enough free storage to unpack it — the App falls back to the previous **Wikidata Query Service** request so that you still see something. This is a failure path, not normal operation.
    - **Putting attractions in a sensible order does still involve Wikidata, and version 1.18.2 reduces it.** OpenStreetMap records where places are but not how well known they are, so for the places it tags with a Wikidata identifier the App looks up how many Wikipedia language editions write about them — that is what decides which attractions are listed first. Until version 1.18.2 that was a request to the **Wikidata Query Service** on most attraction searches. It sends **those place identifiers and your IP address — not your coordinates**. From version 1.18.2 the App answers most of them from the list shipped inside it, so a search around a well-known place typically sends nothing for this at all; only identifiers the shipped list does not hold are still looked up. Answers are remembered on your device for 30 days, so the same places are not asked about again.
...
| Wikidata Query Service (Wikimedia) | **Two uses, both reduced.** (a) *Finding* famous/notable attractions — **fallback only since version 1.18.0**: normally answered from a list shipped inside the App, and contacted only if that list cannot be read on your device. (b) *Ordering* them — how well known a place is, for attractions OpenStreetMap tags with a Wikidata identifier. **Reduced in version 1.18.2**, which answers most of these from the shipped list; answers are then kept on your device for 30 days | (a) Lat/Lon coordinates, IP. (b) Wikidata place identifiers, IP — **not your coordinates** | https://foundation.wikimedia.org/wiki/Policy:Privacy_policy |
