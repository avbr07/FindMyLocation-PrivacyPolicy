...
| Overpass API (OpenStreetMap community) | Nearby fuel stations, restaurants, medical services & local attractions; searches for a kind of place ("grocery stores") and airport codes. Fuel, Food and Hospital only when you tap them; attractions also searched automatically in advance while you are on the Map tab (see "Location Data"). **The famous-places half of an attractions search no longer uses any service** — see version 1.18.0 above | Lat/Lon coordinates + search radius, IP; the kind of place or airport code you type | https://osmfoundation.org/wiki/Privacy_Policy |
| Wikidata Query Service (Wikimedia) | **Fallback only, from version 1.18.0.** Famous/notable attractions now come from a list shipped inside the App and normally involve no request at all. This service is contacted only if that shipped list cannot be read on your device | Lat/Lon coordinates, IP | https://foundation.wikimedia.org/wiki/Policy:Privacy_policy |
