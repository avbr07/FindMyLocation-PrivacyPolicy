...
- If you tap the Map tab's **fuel (Petrol/Gas)**, **Food**, **Hospital** or **Attractions** buttons, your coordinates and chosen search radius are sent to the **Overpass API** (an OpenStreetMap community service) to find nearby fuel stations, restaurants, medical services and attractions.
    - **Famous places are worked out on your device.** The well-known attractions layer (temples, monuments, museums and the like) needs no connection, so your coordinates are **not sent to the Wikidata Query Service** for it. The App still asks the **Overpass API** for the smaller, more local places around you, and for their exact positions.
    - If that cannot be done on your device, the App falls back to a **Wikidata Query Service** request so that you still see something. This is a failure path, not normal operation.
    - **Ordering attractions can involve Wikidata.** For places OpenStreetMap tags with a Wikidata identifier, the App may ask the **Wikidata Query Service** for information it uses to decide the order attractions are listed in. It sends **those place identifiers and your IP address — not your coordinates**. Most searches are answered on your device and send nothing for this. Answers are remembered on your device for 30 days.
...
- **While you are typing, nothing is sent at all.** From version 1.21.0 the search box offers suggestions as you type — towns, cities and well-known places. They are worked out **entirely on your device**, using your approximate position only to decide the order they appear in. **No keystroke, no search text and no coordinate leaves your phone for this**, it works with no connection, and it does not count towards the daily limit described below. Only when you then run a search does the next point apply.
...
- **Trip history stays on your device.** To power the Trips tab, the App keeps a log of your recent journeys (coordinates, place names, and times) **in the App's private storage on your device only**, for your **14 most recent travel days**: a day's trips are deleted automatically once 14 newer days with travel have been recorded (days you don't travel don't shorten this). **You can also choose to keep individual trips indefinitely** by tapping the bookmark on them (up to 100 trips) — a kept trip is exempt from that automatic deletion and remains on your device until **you** delete it, or you uninstall the App. Nothing is kept indefinitely unless you ask for it: every trip is deleted automatically unless you have marked it yourself. This history is **never transmitted** to us or to any third party (viewing a route on the map only fetches map imagery, as above), and it is removed entirely if you uninstall the App. All other location data is held in memory only during active use and is discarded when tracking stops or the app is closed.
...
- The App offers two **optional**, separately controlled features that use location while the screen is locked or the app is in the background. Both are off until you turn them on, both run as an Android **foreground service** with an **ongoing, visible notification** so you always know when they are active, and both can be stopped at any time:
  - **Keep running in background** — keeps updating and announcing your location continuously until you switch it off.
    - **Changed in version 1.19.0.** It now also switches **itself** off when nothing is happening. If your phone has not moved about **100 metres** from where it came to rest for **two hours**, no journey is being recorded, and the App is not on screen, background location updates **stop on their own** and a notification tells you so, with a **Resume** button to turn them straight back on. This only ever *reduces* background location use — nothing new is collected, nothing new is sent, and no new service is contacted. Your step count is unaffected either way. If you would rather it never stopped by itself, there is a switch in Settings to turn this behaviour off.
    - **Changed in version 1.19.1.** Two changes, both about telling you it happened. First, the button on the Location tab now plainly reads **Stopped** and flashes once so you can see that background updates are no longer running — it no longer shows as though it were still on. Second, **while it is stopped, the App uses Android's activity-recognition sensor to notice when you next start walking or driving, and posts a reminder** so you can switch it back on if you want it. That reminder is capped at **three times** for each stop, at least fifteen minutes apart, and stops entirely once you turn background updates back on. This is the same on-device motion sensing already described for trip recording, now also used while background updates are stopped; **it collects no location, sends nothing, and contacts no service.** Nothing starts itself: background location only ever resumes when **you** turn it back on.
  - **Trip recording** — saves your walks and drives to the on-device Trips tab, tracing each journey's route even while the phone is locked or in your pocket. It runs only after you have turned the Trip recording switch on yourself and agreed to the privacy notice that switch shows you, and it never announces or geocodes anything.
    - **Changed in version 1.20.1 — please read.** Two changes, both about who decides. First, **on a new installation trip recording now starts switched OFF.** Nothing is recorded until you turn the switch on, and turning it on is what asks you. If you are updating and had trip recording on, it stays on — an update will not switch off something you were already using. Second, the notice that asks for your agreement is now shown **only at that switch**, and it asks each time you reach for it, rather than once at first launch. The launch-time notice is gone: agreeing to it started nothing, because only the switch actually begins a recording.
...
| Wikidata Query Service (Wikimedia) | Finding and ordering notable attractions. Rarely contacted — most searches are answered on your device | Finding: Lat/Lon coordinates, IP. Ordering: Wikidata place identifiers, IP — **not your coordinates**. Answers kept on your device for 30 days | https://foundation.wikimedia.org/wiki/Policy:Privacy_policy |
...
| ACTIVITY_RECOGNITION | Count steps for the Speed tab's step counter; while trip recording is on, notice when you start moving so a trip can begin even if the phone was locked before you left; and, **from version 1.19.1**, notice when you start moving while background location has stopped itself, so the App can remind you it is off. The sensor reports movement only — it has no access to where you are (optional — Android 10+; denying it only disables step counting, the automatic trip start and that reminder) |
...
- Location data written to storage by the App is limited to three things, all in the App's private storage, all never transmitted, and all removed on uninstall:
  - the **on-device trip history** described above — auto-deleted past your 14 most recent travel days unless you have chosen to keep a particular trip;
...
|------|------------------|----------|
| Trip history (routes, place names, times) | App's private on-device storage | Your **14 most recent travel days** — a day's trips are deleted once 14 newer travel days exist |
| Trips you chose to keep (bookmarked, max 100) | App's private on-device storage | **Until you delete them** — exempt from the automatic 14-day pruning, and never transmitted |
...
- Location is processed **on your device**. Everything the App itself writes to disk — the trip history (your 14 most recent travel days, plus any trips you chose to keep), the place cache, the nearby-places cache, and your preferences — is stored in the App's **private, sandboxed storage**, which Android prevents other apps from reading. A backup or shared trip that **you** create is deliberately outside that sandbox, because the whole point of it is to survive the App being removed; from that moment its safety is in your hands.
