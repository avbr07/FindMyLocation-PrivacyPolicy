...
- The App offers two **optional**, separately controlled features that use location while the screen is locked or the app is in the background. Both are off until you turn them on, both run as an Android **foreground service** with an **ongoing, visible notification** so you always know when they are active, and both can be stopped at any time:
  ...
  - **Trip recording** — saves your walks and drives to the on-device Trips tab, tracing each journey's route even while the phone is locked or in your pocket. It runs only after you have confirmed it (a one-time prompt on your first trip, or by turning the Trip recording switch on yourself), and it never announces or geocodes anything.
    ...
    - **Changed in version 1.17.0 — please read.** The App now asks for Android's separate **background location** permission ("Allow all the time"), immediately after — and only after — you confirm the one-time screen-locked recording prompt. It is never requested at first launch and never bundled with the permissions asked for when you open the App. Granting it fixes the case described above from the other side: a journey that **begins while the phone is already locked** can now start tracing on its own, rather than being recorded as a straight line from wherever you happened to last be seen. **Declining changes nothing else** — every other feature behaves exactly as before, and you can grant or revoke it at any time from the Location group in the App's Settings, or Android's own app settings. It is used for the trip recording described here and for nothing else.
    - The same Settings group also links to Android's **battery optimisation** screen. This is a convenience shortcut only: the App requests no special battery permission, and exempting it simply stops Android delaying location updates while the screen is off, which on some phones distorts a recorded route. Nothing about what is collected, stored or transmitted changes either way.
...
| **On-device elevation grid (ETOPO 2022, NOAA NCEI)** | Ground elevation for "flight height" | **Nothing — the data ships inside the App and never leaves your device** | Public domain (https://www.ncei.noaa.gov/) |
...
| Overpass API (OpenStreetMap community) | Nearby fuel stations, restaurants, medical services & attractions; searches for a kind of place ("grocery stores") and airport codes. Fuel, Food and Hospital only when you tap them; attractions also searched automatically in advance while you are on the Map tab (see "Location Data") | Lat/Lon coordinates + search radius, IP; the kind of place or airport code you type | https://osmfoundation.org/wiki/Privacy_Policy |
...
| ACCESS_BACKGROUND_LOCATION | Let trip recording begin tracing a journey that starts while the phone is already locked, instead of only one begun with the App on screen. Requested separately, and only after you confirm the one-time screen-locked recording prompt — never at first launch (optional — Android 10+; denying it leaves every other feature working exactly as before) |
